...
def update_elo_2v2(team1_ratings, team2_ratings, result, K=32, ratio = 400):
    """
    ...
    if result[0] > result[1]:
        for rating in team1_ratings:
            ...
            team1_new_ratings.append(int(round(50+K * (game_result - team1_P))))  # Equipo 1
        for rating in team2_ratings:
            team2_P = 1/ (1 + 1 * math.pow(10, 1 * (team1_avg - rating) / ratio))
            team2_new_ratings.append(int(round(-50 + K * (1-game_result - team2_P))))
    else:
        for rating in team1_ratings:
            team1_P = 1/ (1 + 1 * math.pow(10, 1 * (team2_avg - rating) / ratio))
            team1_new_ratings.append(int(round(-50+K * (game_result - team1_P))))  # Equipo 1
